...
`neon-http`와 `neon-websockets` 드라이버를 사용하면 서버리스 환경에서 TCP 대신 HTTP 또는 WebSocket을 통해 Neon 데이터베이스에 액세스할 수 있습니다. HTTP를 통한 쿼리는 단일 비대화형 트랜잭션에 더 빠릅니다.

세션이나 대화형 트랜잭션 지원이 필요하거나, `pg` 드라이버와 완전히 호환되는 대체 드라이버가 필요한 경우 WebSocket 기반 `neon-serverless` 드라이버를 사용할 수 있습니다. [Postgres](/docs/get-started/postgresql-new)를 사용하여 Neon 데이터베이스에 직접 연결할 수도 있습니다.
