...
# Drizzle과 SingleStore 시작하기

<Prerequisites>
  - **dotenv** - 환경 변수 관리 패키지 - [자세히 보기](https://www.npmjs.com/package/dotenv)
  - **tsx** - TypeScript 파일 실행 패키지 - [자세히 보기](https://tsx.is/)
  - **mysql2** - MySQL 데이터베이스 쿼리 패키지 - [자세히 보기](https://github.com/sidorares/node-mysql2)
</Prerequisites>

SingleStore 데이터베이스에서 Drizzle을 사용하려면 `singlestore` 드라이버를 사용해야 합니다.

**[공식 웹사이트](https://github.com/sidorares/node-mysql2)**에 따르면,
`mysql2`는 성능에 중점을 둔 Node.js용 MySQL 클라이언트입니다.

Drizzle ORM은 SingleStore 데이터베이스를 위해 `drizzle-orm/singlestore` 패키지와 함께 `mysql2`를 기본적으로 지원합니다.
