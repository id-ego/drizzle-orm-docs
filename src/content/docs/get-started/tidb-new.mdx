...
<Prerequisites>
  ...
  - **serverless-js** - HTTP 외부 연결이 필요한 서버리스 및 엣지 컴퓨팅 플랫폼용 패키지 - [자세히 보기](https://github.com/tidbcloud/serverless-js)
</Prerequisites>

<Callout title='중요' type='warning'>
이 튜토리얼에서는 **HTTP** 호출을 수행하기 위해 `@tidbcloud/serverless` 드라이버를 사용합니다. TCP를 통해 TiDB에 연결해야 하는 경우 [MySQL 시작하기](/docs/get-started/mysql-new) 페이지를 참조하세요.
</Callout>
