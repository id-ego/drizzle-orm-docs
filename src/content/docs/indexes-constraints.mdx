...
`UNIQUE`와 `PRIMARY KEY` 제약조건 모두 컬럼 또는 컬럼 집합의 고유성을 보장합니다.
...
`PRIMARY KEY`와 마찬가지로, 복합 기본 키는 여러 필드를 사용하여 테이블의 각 레코드를 고유하게 식별합니다.
...
<Tabs items={['PostgreSQL', 'MySQL', 'SQLite', 'SingleStore', 'MSSQL', 'CockroachDB']}>
  <Tab>
    ...
    <Callout type="warning" emoji="⚠️">
      `drizzle-kit@0.22.0` 및 `drizzle-orm@0.31.0` 이전 버전에서는 `drizzle-kit`이 인덱스 `name`과 `on()` 파라미터만 지원합니다.
...
```
  ...
  <Tab>
    ...
    <Callout type="warning" emoji="⚠️">
      현재 `drizzle-kit`은 인덱스 `name`과 `on()` 파라미터만 지원합니다.
    </Callout>
