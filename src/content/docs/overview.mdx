...
**[관계형](/docs/rqb) API와 [SQL-like](/docs/select) 쿼리 API를 모두 갖춘 유일한 ORM**으로,
관계형 데이터에 접근할 때 두 방식의 장점을 모두 누릴 수 있습니다.
Drizzle는 가볍고, 고성능이며, 타입 안전하고, 유연하며 **처음부터 서버리스를 염두에 두고 설계**되었습니다.
...
우리는 이것이 레거시 명명법에 기반한 오해라고 생각하며, 이러한 도구들을 **데이터 프레임워크**라고 부르는 것이 적절하다고 봅니다.

<Callout type="error" emoji="️💔">
  데이터 프레임워크는 프레임워크를 **중심으로** 프로젝트를 구축해야 하며, 프레임워크**와 함께** 구축하는 것이 아닙니다.
</Callout>

**Drizzle**은 프로젝트나 구조를 방해하지 않고 원하는 방식으로 프로젝트를 구축할 수 있게 해줍니다.

Drizzle를 사용하면 TypeScript로 데이터베이스 스키마를 정의하고 관리할 수 있으며, SQL-like 방식이나
...
다른 ORM과 데이터 프레임워크들은 SQL로부터 벗어나거나 추상화하려는 경향이 있어,
SQL과 프레임워크의 API를 모두 배워야 하는 이중 학습 곡선이 발생합니다.

Drizzle은 그 반대입니다.
우리는 SQL을 수용하고 Drizzle을 핵심부터 SQL-like로 구축하여, 학습 곡선이 거의 없거나
전혀 없으면서도 SQL의 모든 기능을 활용할 수 있도록 했습니다.
...
## 왜 SQL-like가 아닌가?
우리는 항상 완벽하게 균형 잡힌 솔루션을 추구합니다. SQL-like가 대부분의 요구사항을 충족하지만,
더 나은 방식으로 데이터를 쿼리할 수 있는 특정한 일반적 시나리오들이 있습니다.

우리는 여러분을 위해 **[Queries API](/docs/rqb)**를 구축했습니다. 이를 통해 데이터베이스에서 중첩된 관계형 데이터를
가장 편리하고 성능이 뛰어난 방식으로 가져올 수 있으며, 조인이나 데이터 매핑에 대해 고민할 필요가 없습니다.
...
Drizzle은 업계 표준 데이터베이스 드라이버를 통해 기본적으로 작동합니다. 모든 주요 **[PostgreSQL](/docs/get-started-postgresql)**, **[MySQL](/docs/get-started-mysql)**, **[SQLite](/docs/get-started-sqlite)** 또는 **[SingleStore](/docs/get-started-singlestore)** 드라이버를 지원하며, **[정말 빠르게](https://twitter.com/DrizzleORM/status/1653082492742647811?s=20)** 새로운 드라이버를 추가하고 있습니다.
...
뛰어난 **[Discord 커뮤니티](https://driz.link/discord)**가 있으며, 모든 개발자를 **[Twitter](https://twitter.com/drizzleorm)**에서 환영합니다.

이제 Drizzle과 **[PostgreSQL](/docs/get-started-postgresql)**, **[MySQL](/docs/get-started-mysql)** 또는 **[SQLite](/docs/get-started-sqlite)** 데이터베이스로 멋진 것을 만들어보세요 🚀
