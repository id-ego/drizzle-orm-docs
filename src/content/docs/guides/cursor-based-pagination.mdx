...
- [order by 절](/docs/select#order-by)을 사용한 [Select 문](/docs/select)
- [order by 절](/docs/rqb#order-by)을 사용한 [관계형 쿼리](/docs/rqb)
...
순차적이지 않은 기본 키를 사용하는 경우 (예: `UUIDv4`), 순차적인 컬럼 (예: `created_at` 컬럼)을 추가하고 여러 커서를 사용해야 합니다.
다음과 같이 할 수 있습니다:
...
따라서 특정 페이지로 직접 이동해야 하거나 더 간단한 페이지네이션 구현이 필요한 경우 [offset/limit](/docs/guides/limit-offset-pagination) 페이지네이션을 사용하는 것을 고려해야 합니다.
