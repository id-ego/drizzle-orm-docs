...
PostgreSQL은 `point`라는 기하학 데이터를 저장하기 위한 특수한 데이터타입을 제공합니다. 이는 2차원 공간의 점을 나타내는 데 사용됩니다. point 데이터타입은 `(x, y)` 좌표 쌍으로 표현됩니다.
...
`getColumns`는 `drizzle-orm@1.0.0-beta.2`부터 사용 가능합니다(자세한 내용은 [여기](/docs/upgrade-v1)를 참고하세요)
...
두 개의 대각선 점으로 정의된 직사각형 경계 내에 `point` 타입의 `location`이 포함된 행만 필터링하려면 `<@` 연산자를 사용할 수 있습니다. 이 연산자는 첫 번째 객체가 두 번째 객체에 포함되거나 그 위에 있는지 확인합니다:
