...
PostgreSQL과 SQLite에서 Drizzle을 사용하여 upsert 쿼리를 구현하려면 ([MySQL](/docs/guides/upsert#mysql)로 건너뛰기) `.onConflictDoUpdate()` 메서드를 사용할 수 있습니다:
...
PostgreSQL과 SQLite에서 한 번의 쿼리로 여러 행을 upsert하려면 `sql operator`와 `excluded` 키워드를 사용할 수 있습니다. `excluded`는 삽입이 제안되었지만 충돌로 인해 삽입되지 않은 행을 참조하는 특수 참조입니다.
...
Drizzle은 간단하고 유연한 API를 가지고 있어 커스텀 솔루션을 쉽게 만들 수 있습니다. PostgreSQL과 SQLite에서 충돌로 인해 여러 행의 특정 컬럼을 업데이트하는 커스텀 함수를 만드는 방법은 다음과 같습니다:

<Callout type='warning'>
`getColumns`는 `drizzle-orm@1.0.0-beta.2`부터 사용 가능합니다 ([자세히 보기](/docs/upgrade-v1))

버전 1 이전 버전(예: `0.45.1`)을 사용 중이라면 `getTableColumns`를 사용하세요
</Callout>
...
`update` 문에 `where` 절을 포함한 upsert 쿼리를 구현하려면 `onConflictDoUpdate` 메서드의 `setWhere` 속성을 사용할 수 있습니다:
...
특정 컬럼을 제외한 모든 컬럼을 업데이트하려면 다음과 같이 이전 값을 유지할 수 있습니다:
...
MySQL에서 Drizzle을 사용하여 upsert 쿼리를 구현하려면 `.onDuplicateKeyUpdate()` 메서드를 사용할 수 있습니다. MySQL은 기본 키와 고유 인덱스를 기반으로 충돌 타겟을 자동으로 결정하며, 고유 인덱스가 충돌하면 행을 업데이트합니다.
...
MySQL에서 한 번의 쿼리로 여러 행을 upsert하려면 `sql operator`와 `values()` 함수를 사용할 수 있습니다. `values()` 함수는 중복 키 충돌이 발생하지 않았다면 삽입되었을 컬럼의 값을 참조합니다.  
...
Drizzle은 간단하고 유연한 API를 가지고 있어 커스텀 솔루션을 쉽게 만들 수 있습니다. MySQL에서 충돌로 인해 여러 행의 특정 컬럼을 업데이트하는 커스텀 함수를 만드는 방법은 다음과 같습니다:
...
await db
  ...
  .values(data)
  .onDuplicateKeyUpdate({
    set: { ...data, email: sql`${users.email}` }, // email은 이전 값 유지
});
```
