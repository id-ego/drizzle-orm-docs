...
이 가이드는 Drizzle ORM을 사용하여 PostgreSQL에서 전문 검색(full-text search)을 구현하는 방법을 설명합니다. 전문 검색은 문서 또는 문서 집합 내에서 텍스트를 검색하는 기술입니다. 문서는 전문 검색 시스템에서 검색의 단위입니다. PostgreSQL은 `to_tsvector` 및 `to_tsquery`와 같은 전문 검색 작업을 위한 함수 세트를 제공합니다:

`to_tsvector` 함수는 텍스트 문서를 토큰으로 파싱하고, 토큰을 어휘소(lexeme)로 축소한 후, 문서 내 위치와 함께 어휘소를 나열하는 `tsvector`를 반환합니다:
...
현재 Drizzle은 `tsvector` 타입을 기본적으로 지원하지 않으므로, `text` 컬럼의 데이터를 즉시 변환해야 합니다. 성능을 향상시키기 위해 다음과 같이 컬럼에 `GIN` 인덱스를 생성할 수 있습니다:
...
Drizzle ORM을 사용하여 PostgreSQL에서 전문 검색을 구현하려면 `sql` 연산자와 함께 `to_tsvector` 및 `to_tsquery` 함수를 사용할 수 있습니다:
...
키워드 중 하나라도 일치시키려면 `|` 연산자를 사용할 수 있습니다:
...
웹 검색 엔진에서 사용하는 것과 유사한 대체 구문을 가진 `to_tsquery`의 간소화된 버전인 `websearch_to_tsquery` 함수를 사용할 수도 있습니다:
...
여러 컬럼에 대해 전문 검색을 구현하려면 여러 컬럼에 인덱스를 생성하고 `to_tsvector` 함수로 컬럼을 연결할 수 있습니다:
...
`setweight` 함수는 tsvector의 항목에 주어진 가중치로 레이블을 지정하는 데 사용되며, 가중치는 A, B, C 또는 D 중 하나입니다. 이는 일반적으로 제목과 본문과 같이 문서의 다른 부분에서 오는 항목을 표시하는 데 사용됩니다.

다음과 같이 여러 컬럼에 대해 쿼리할 수 있습니다:
...
`getColumns`는 `drizzle-orm@1.0.0-beta.2`부터 사용할 수 있습니다(자세한 내용은 [여기](/docs/upgrade-v1)를 참조하세요)

`0.45.1`과 같은 pre-1 버전을 사용 중이라면 `getTableColumns`를 사용하세요
</Callout>
...
`ts_rank` 함수는 문서 전체에서 쿼리 용어의 빈도에 중점을 둡니다. `ts_rank_cd` 함수는 문서 내에서 쿼리 용어의 근접성에 중점을 둡니다.
