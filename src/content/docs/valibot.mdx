...
`drizzle-valibot`은 **[Drizzle ORM](https://github.com/drizzle-team/drizzle-orm)** 스키마로부터 **[Valibot](https://valibot.dev/)** 스키마를 생성할 수 있게 해주는 플러그인입니다.
...
이 문서는 `drizzle-valibot@0.3.0` 이상 버전을 대상으로 합니다.

Drizzle ORM v0.36.0 이상과 Valibot v1.0.0-beta.7 이상도 함께 설치되어 있어야 합니다.
</Callout>
...
각 스키마 생성 함수는 필드의 스키마를 확장, 수정 또는 완전히 덮어쓸 수 있는 추가 선택적 매개변수를 받습니다. 콜백 함수를 정의하면 스키마가 확장 또는 수정되고, Valibot 스키마를 직접 제공하면 기존 스키마를 덮어씁니다.
...
const userSelectSchema = createSelectSchema(users, {
  ...
  bio: (schema) => pipe(schema, maxLength(1000)), // nullable/optional이 적용되기 전에 스키마 확장
  preferences: object({ theme: string() }) // nullability를 포함하여 필드 전체를 덮어씀
});
